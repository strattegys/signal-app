...
${styleContext}`;
    ...
    doc.fontSize(7).fillColor("#00e5ff").font("Courier-Bold").text("PRODUCED BY SIGNAL // AI_CORE_v1.0", doc.page.width - 250, 35, { align: "right", width: 200 });
    ...
    if (sections.imagePrompt) {
      try {
        ...
        if (imagePart?.inlineData?.data) {
          ...
          doc.image(imgBuffer, xOffset, imageY, { width: imgWidth, height: imgHeight });
          ...
          doc.fontSize(10).fillColor("#444444").font("Helvetica-Oblique").text(sections.imagePrompt.split(/[.!?]/)[0] + ".", 58, descY + 20, { width: pageWidth - 20 });
        }
      } catch (err) {
        ...
        if (sections.imagePrompt) {
          ...
          doc.fontSize(10).fillColor("#444444").font("Helvetica-Oblique").text(sections.imagePrompt.split(/[.!?]/)[0] + ".", 58, y + 20, { width: pageWidth - 20 });
        }
      }
    }
...
function parseBrandPackage(text: string): {
  ...
  const result = { audience: "", pitch: "", imagePrompt: "" };
